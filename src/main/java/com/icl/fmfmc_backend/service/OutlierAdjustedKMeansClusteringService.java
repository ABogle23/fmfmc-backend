...
public class OutlierAdjustedKMeansClusteringService implements ClusteringStrategy {
    ...
    private static List<Point> recalculateCentroids(List<List<Point>> clusters) {
        ...
        for (List<Point> cluster : clusters) {
            ...
            centroids.add(geometryFactory.createPoint(new Coordinate(sumX / totalWeight, sumY / totalWeight)));
        }
        return centroids;
    }
    ...
    private static List<Point> filterOutliersByZScore(List<Point> points) {
        double mean = points.stream().mapToDouble(p -> p.distance(geometryFactory.createPoint(new Coordinate(0, 0)))).average().orElse(0);
        double stdev = Math.sqrt(points.stream().mapToDouble(p -> Math.pow(p.distance(geometryFactory.createPoint(new Coordinate(0, 0))) - mean, 2)).average().orElse(0));

        return points.stream()
                .filter(p -> Math.abs(p.distance(geometryFactory.createPoint(new Coordinate(0, 0))) - mean) / stdev < 3)
                .collect(Collectors.toList());
    }
    ...
    private static void refineClusters(List<List<Point>> clusters) {
        for (List<Point> cluster : clusters) {
            ...
            double avgDistance = cluster.stream().mapToDouble(p -> p.distance(centroid)).average().orElse(0);
    ...
    private static Point calculateCentroid(List<Point> cluster) {
        ...
        return geometryFactory.createPoint(new Coordinate(sumX / cluster.size(), sumY / cluster.size()));
    }
