...
public class TestHelperFunctions {
  ...
  public static void setBatteryAndCharging(
    ...
    journey.setChargeLevelAfterEachStop(
        journey.getEvRange() * journey.getChargeLevelAfterEachStopPct());
  ...
  public static void setFoodAdjacentCharger(
      List<Charger> chargers, Journey journey, Long chargerId) {

    journey.setStopForEating(true);

    Optional<Charger> foodAdjacentCharger =
        chargers.stream().filter(charger -> Objects.equals(charger.getId(), chargerId)).findFirst();
